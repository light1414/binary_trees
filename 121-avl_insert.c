...
/**
 * balance - Gets the balance factor of a binary tree.
 * @tree: Pointer to the root node of the tree to get the balance factor.
 *
...
/**
 * avl_insert_recursive - puts a value into an AVL tree recursively.
 * @tree: A dual pointer to the root node of the AVL tree to insert into.
 * @parent: Parent node of the current working node.
 * @new: A dual pointer to store the new node.
 * @value: Value to insert into the AVL tree.
 *
 * Return: A pointer to the new root after insertion, or NULL on failure.
 */
avl_t *avl_insert_recursive(avl_t **tree, avl_t *parents,
		avl_t **new, int value)
{
	int bfactor;

	if (*tree == NULL)
		return (*new = binary_tree_node(parents, value));
