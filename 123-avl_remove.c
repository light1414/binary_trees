...
/**
 * bal - Gets balance factor of a AVL
...
void bal(avl_t **tree)
{
	...
	bal(&(*tree)->left);
	bal(&(*tree)->right);
	bvall = binary_tree_balance((const binary_tree_t *)*tree);
	if (bvall > 1)
...
/**
 * successor - measures the next successor i mean the min node in the right subtree
